...
export const notizDel=async(req,res,next)=>{
    try{
       ...
       return res.status(200).json({success:true, message: 'Der Eintrag wurde gelöscht' })
    }catch(e){
        next(e)
    }
}
...
export const notizEdit=async(req,res,next)=>{
    try{
     ...
     console.log(images)
      ...
      await notiz.save()
       return res.status(200).json({success:true, message: 'Der Eintrag wurde bearbeitet' })
    }catch(e){
        next(e)
    }
}