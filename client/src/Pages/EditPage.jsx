...
export const EditPage = () => {
  ...
  async function handleOnSubmit(e) {
    ...
    if (descriptionText.length < 2) {
      ...
      setTimeout(() => {
        setFormError(false), setAlertText("");
      ...
      {notiz && (
        <div className=" min-h-screen my-20 ">
          <form
            ...
            className="flex flex-col items-center justify-center w-2/4 mx-auto mt-12 max-sm:w-4/5"
          >
              ...
              {imagesUrl &&
                ...
                imagesUrl.map((image, index) => (
                  <div className="w-60 relative" key={`url-${index}`}>
                    ...
                    />
                    <TiDelete
                      onClick={() => handleOnClickDeleteImageUrl(index)}
          ...
          <NavLink
            to="/home"
            className="drop-shadow-lg px-4  py-1 rounded bg-rose-700  text-white sm:hover:bg-red-900 w-20 fixed bottom-20 right-7 max-sm:hidden"
          >
            Zurück
          </NavLink>
        </div>
